 ...
 * Time: 11:32
 */
require_once "WechatException.php";
...
class WechatPayApi{
    ...
    public static function unifiedOrder($inputObj, $timeOut = 6)
    {
        ...
        if(!$inputObj->IsNotify_urlSet()){
            $inputObj->SetNotify_url(C('Wechat.NOTIFY_URL'));//异步通知url
        }

        $inputObj->SetAppid(C('Wechat.APPID'));//公众账号ID
        $inputObj->SetMch_id(C('Wechat.MCHID'));//商户号
    ...
    {
        //如果不需要上报数据
        if(C('Wechat.REPORT_LEVENL') == 0){
            return;
        }
        //如果仅失败上报
        if(('Wechat.REPORT_LEVENLC') == 1 &&
    ...
    private static function postXmlCurl($xml, $url, $useCert = false, $second = 30)
    {
        ...
        //如果有配置代理这里就设置代理
        if(C('Wechat.CURL_PROXY_HOST') != "0.0.0.0"
            &&C('Wechat.CURL_PROXY_PORT') != 0){
            curl_setopt($ch,CURLOPT_PROXY, C('Wechat.CURL_PROXY_HOST'));
            curl_setopt($ch,CURLOPT_PROXYPORT,C('Wechat.CURL_PROXY_PORT'));
        }
        ...
        if($useCert == true){
            ...
            curl_setopt($ch,CURLOPT_SSLCERT, C('Wechat.SSLCERT_PATH'));
            curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
            curl_setopt($ch,CURLOPT_SSLKEY,C('Wechat.SSLKEY_PATHC'));
        }
        //post提交方式
